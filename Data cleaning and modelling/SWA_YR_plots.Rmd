...
timecoursedata <- timecoursedata %>% 
  ...
  mutate(timestamp_zero = timestamp - timestamp[1], 
         ...
         World_x_mirrored = World_x * sign(heading + 1e-6),
         World_z_mirrored = World_z * sign(heading + 1e-6),
...
ggplot(timecoursedata %>%
  ...
  #theme_gray() +
  xlab("Time (s)") +
  ylab("Steering wheel angle (degrees/s)") +
  guides(color = guide_legend(title = "Heading (degrees)")) +
  theme(axis.title.x = element_text(size = 20), axis.text.x = element_text(size = 20), axis.title.y = element_text(size = 20), axis.text.y = element_text(size = 20), title = element_text(size = 20), legend.title = element_text(size = 20), legend.text = element_text(size = 20),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  geom_vline(aes(xintercept = 2.2, linetype = "Road-line appears")) +
  ggtitle("Steering wheel angle over time for each heading") +
  ggsave('SWA.png', plot = last_plot(), width = 35, height = 16, units = 'cm', dpi = 300, type = 'cairo')
...
ggplot(timecoursedata %>%
  ...
  #theme_gray() +
  xlab("Time (s)") +
  ylab("Yaw rate change (degrees/s/s)") +
  guides(color = guide_legend(title = "Heading (degrees)")) +
  theme(axis.title.x = element_text(size = 16), axis.text.x = element_text(size = 15), axis.title.y = element_text(size = 15), axis.text.y = element_text(size = 16), title = element_text(size = 12), legend.title = element_text(size = 15), legend.text = element_text(size = 15), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  geom_vline(aes(xintercept = 2.2, linetype = "Road-line appears")) +
  ggtitle("Steering magnitude over time for each heading") 
...
exampletimecoursedata <- timecoursedata %>%
  dplyr::filter(StraightVisible == 2)

ggplot(dplyr::filter(exampletimecoursedata, ppid_trialn == "10_1_84"), aes(x = timestamp_zero, y = World_x_mirrored)) +
  geom_point()
