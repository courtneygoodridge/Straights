...
temp = list.files(pattern = c("BenLui17_1")) # list all CSV files in the directory
...
upperthreshold = 0.140 # 0.105 #  # upper threshold for consistent steering response
...
workingdatathresholdUnsmooth <- workingdatathresholdUnsmooth %>%
  filter(Genuine_Response == TRUE)
#  & EarlyResponses == FALSE
...
ggplot(data = workingdatatimecourseUnsmooth, aes(x = anchored_timestamp, y = YawRateChange, colour = ppid_trialn)) +
  ...
  geom_vline(data = workingdatathresholdUnsmooth %>%
               summarise(medianFirstSteeringTime = median(FirstSteeringTime, na.rm = TRUE)), aes(xintercept = medianFirstSteeringTime, color = "median")) +
  geom_vline(data = workingdatathresholdUnsmooth %>%
               summarise(meanFirstSteeringTime = mean(FirstSteeringTime, na.rm = TRUE)), aes(xintercept = meanFirstSteeringTime, color = "mean")) +
  scale_color_manual(name = "Reaction times", values = c(median = "blue", mean = "red")) +
  facet_wrap( ~ heading)
...
ggplot(data = workingdatatimecourseUnsmooth, aes(x = anchored_timestamp, y = YawRateChange, colour = ppid_trialn)) +
  ...
  scale_x_continuous(name="Timestamp", limits=c(0, 2.5))
...
overall <- workingdatathresholdUnsmooth %>%
  group_by(heading) %>%
...
```{r first steering time graphed example, fig.height = 18, fig.width = 26}
library(dplyr)
library(ggplot2)
...
ggplot(data = filter(workingdatatimecourseUnsmooth, heading == 2), aes(x = anchored_timestamp, y = YawRateChange)) +
  ...
  geom_vline(data = filter(workingdatathresholdUnsmooth, heading == 2, EarlyResponses == TRUE), aes(xintercept = FirstSteeringTime), linetype = "dashed") +
  facet_wrap( ~ ppid_trialn) +
  scale_y_continuous(limits = c(-0.60, 0.80)) +
  ggtitle("Visualisation of data pre-processing for +2 heading") +
  xlab("Timestamp (secs)") +
  ylab("Yaw Rate Change") +
  labs(color = "Genuine Response") 
