...
ggplot(timecoursedata %>%
         dplyr::filter(heading > 0) %>%
         dplyr::filter(frame <= 279) %>%
         group_by(heading, frame) %>%
         summarise(meanSteeringbias = mean(steeringbias)) %>%
         mutate(heading_factor = as.factor(heading)) %>%
         mutate(timestamp = frame / 60), aes(x = timestamp, y = meanSteeringbias, colour = heading_factor)) + 
  geom_line() +
  ylim(-0.45, 0.45)



### Example of geom_path use from Will's report ###
