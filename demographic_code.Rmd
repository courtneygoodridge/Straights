...
temp = list.files(pattern = c("BenLui17_4", "*.csv")) # list all CSV files in the directory
...
workingdata <- workingdata %>%
  group_by(trialn) %>%
  mutate(gender = 1, age = 25, licenseTime = 98, contacts_glasses = FALSE) %>% # female = 1, male = 2; license time in months
  ungroup()
...
write.csv(workingdata, file = "BenLui17_4_full.csv")
