...
```{r filtering timecourses for steering magnitude}
library(dplyr)

workingdatatimecourseFiltered <- workingdatatimecourseUnsmooth %>%
  group_by(ppid_trialn) %>%
  filter(max(abs(YawRateChange)) > upperthreshold, min(abs(YawRateChange)) < lowerthreshold)
# Filters timecourse data so only genuine timecourses are passed through to steering magnitude calculation.

workingdatatimecourseFiltered %>%
  filter(heading == 0) %>%
  summarise(trials = length(unique(ppid_trialn)))
...
avgtimecourse <- workingdatatimecourseUnsmooth %>%
  group_by(heading_collapsed, frame) %>%
  summarise(meanYawRateChange = mean(YawRateChange))

avgtimecourse <- spread(avgtimecourse, heading_collapsed, meanYawRateChange)
