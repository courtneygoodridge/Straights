...
temp = list.files(pattern = c("BenLui17_16_", "*.csv")) # list all CSV files in the directory
...
workingdata <- workingdata %>%
  group_by(trialn) %>%
  mutate(gender = 2, age = 26, licenseTime = 97, contacts_glasses = FALSE, pNum = 16, handedness = 1) %>% # female = 1, male = 2; license time in months; right = 1, left = 2
  ungroup()
...
write.csv(workingdata, file = "BenLui17_16_full.csv")
