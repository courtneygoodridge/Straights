...
temp = list.files(pattern = c("BenLui17_13_", "*.csv")) # list all CSV files in the directory
...
workingdata <- workingdata %>%
  group_by(trialn) %>%
  mutate(gender = 1, age = 24, licenseTime = 72, contacts_glasses = FALSE, pNum = 13, handedness = 1) %>% # female = 1, male = 2; license time in months; right = 1, left = 2
  ungroup()
...
temp = list.files(pattern = c("BenLui17_13_full")) # list all CSV files in the directory
myfiles = lapply(temp, read.csv) # read these CSV in the directory
workingdata <- do.call(rbind.data.frame, myfiles) # convert and combine the CSV files into dataframe
...
write.csv(workingdata, file = "BenLui17_13_full.csv")
