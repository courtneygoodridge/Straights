...
group_by(ppid_trialn) %>%
  ...
  ungroup() %>%
  transmute(ppid_trialn, heading, StartFrame = frame, StartYaw = YawRateChange, FirstSteeringTime = anchored_timestamp) %>%
...
ungroup() %>%
transmute(ppid_trialn, heading, PeakFrame = frame, PeakYaw = YawRateChange, PeakSteeringTime = anchored_timestamp)
...
```{r number of TRUEtrials per condition for}

workingdatathresholdUnsmooth %>%
  group_by(heading) %>%
  summarise(headingnumber = n())
...
magnitudedata %>%
  group_by(heading) %>%
  summarise(AveragePeakYaw = mean(PeakYaw), MedianPeakYaw = median(PeakYaw), AverageStartYaw = mean(StartYaw), AverageStartTime = mean(FirstSteeringTime), AveragePeakTime = mean(PeakSteeringTime)) %>%
  mutate(AverageYawIncrease = AveragePeakYaw - AverageStartYaw, AverageTimeToPeak = AveragePeakTime - AverageStartTime)
