...
temp = list.files(pattern = c("BenLui17_18")) # list all CSV files in the directory
...
select(ppid_trialn, heading, frame, YawRateChange, anchored_timestamp, pNum) %>% 
group_by(ppid_trialn) %>%
...
a <- nonresponses %>%
  select(ppid_trialn, heading, pNum, response, response_num, FirstSteeringTime)
...
dat <- as.data.frame(cbind(ppid_trialn, heading, pNum, response, response_num, FirstSteeringTime))

rm(ppid_trialn, heading, pNum, response, response_num, FirstSteeringTime, nonresponses, responseid, a, b)
