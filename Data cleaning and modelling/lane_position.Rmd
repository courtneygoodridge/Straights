...
modellingdata %>%
  group_by(heading) %>%
  summarise(meanLateralPosition = mean(individual_threshold))
...
ggplot(timecoursedata, aes(x = timestamp_zero, y = SWA_mirrored, group = ppid_trialn, col = ppid_trialn)) +
  ...
  ggtitle("Plot 1: SWA across multiple timecourses for each heading")

# computes average SWA across the time course
avgtimecourse <- timecoursedata %>%
  filter(frame <= 270) %>%
  group_by(abs_heading, frame) %>%
  summarise(meanSWA = mean(SWA_mirrored)) %>%
...
ggplot(data = avgtimecourse, aes(x = timestamp, y = meanSWA, colour = heading_factor)) + 
  geom_line() +
  ggtitle("Plot 2: average SWA over time for each heading")


# computes WorldYaw across timecourse for individual
ggplot(dplyr::filter(timecoursedata, ppid_trialn == "1_3_88"), aes(x = timestamp_zero, y = WorldYaw)) +
  geom_line() + 
  guides(colour = FALSE) +
  ggtitle("Plot 3: plotting WorldYaw across timecourses")
