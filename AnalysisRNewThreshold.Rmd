...
temp = list.files(pattern = c("BenLui17_t", "*.csv")) # list all CSV files in the directory
...
workingdatatimecourseUnsmooth <- workingdata %>%
  ...
  filter(anchored_timestamp <= min(anchored_timestamp) + 5.0) %>% # add for seconds fro the original data 
...
workingdatatimecourseUnsmooth <- workingdatatimecourseUnsmooth %>%
  ...
  mutate(initialSWA = head(SWA, n = 1)) %>%
  mutate(absInitialSWA = abs(initialSWA)) %>%
  ungroup()
...
workingdatathresholdSmooth <- workingdatatimecourseSmooth %>%
  ...
  ungroup() %>%
  transmute(ppid_trialn, heading, cameraoffset, SWAThres = SWA, FirstSteeringTime = anchored_timestamp, ThresWorld_x = World_x, ThresWorld_z = World_z, ThresWorldYaw = WorldYaw, ThresYawRate_seconds = YawRate_seconds, ThresYawRateChange = smoothedYawRateChange, Genuine_Response = Genuine_Response, PeakMagnitude = PeakMagnitude, initialSWA = initialSWA, absInitialSWA = absInitialSWA) %>%
  group_by(heading) %>%
  mutate(LateResponse = abs(FirstSteeringTime - mean(FirstSteeringTime)) > sd(FirstSteeringTime) & abs(FirstSteeringTime - mean(FirstSteeringTime)) > - sd(FirstSteeringTime))
# returns TRUE if response is more than 1 SD above or below mean first steering time
# guards against too fast/low responses
...
# histogram normality
ggplot(clean_smooth_data, aes(x = FirstSteeringTime)) +
  geom_histogram() +
  facet_wrap( ~ heading)

# shapiro-wilk test for each heading - significant means not normally distributed
clean_smooth_data_heading2 <- filter(clean_smooth_data, heading == 1)
shapiro.test(clean_smooth_data_heading2$FirstSteeringTime)
...
clean_smooth_data$heading_fact<-as.factor(clean_smooth_data$heading)
leveneTest(FirstSteeringTime ~ heading_fact, data = clean_smooth_data)
