...
workingdata <- workingdata %>%
  group_by(ppid_trialn) %>%
  mutate(yawrate_smooth = sgolayfilt(YawRateChange)) %>%
  ungroup()

ggplot(data = dplyr::filter(workingdata, heading == 2), aes(x = anchored_timestamp, y = YawRateChange, colour = ppid_trialn)) +
  geom_line(show.legend = FALSE)

ggplot(data = dplyr::filter(workingdata, heading == 2), aes(x = anchored_timestamp, y = yawrate_smooth, colour = ppid_trialn)) +
  geom_line(show.legend = FALSE) 
