...
timecoursedata <- timecoursedata %>% 
  ...
  mutate(timestamp_zero = timestamp - timestamp[1],
         anchored_timestamp = timestamp - min(timestamp), 
...
ggplot(timecoursedata, aes(x = timestamp_zero, y = SWA_mirrored, group = ppid_trialn, col = ppid_trialn)) +
  ...
  ggtitle("SWA across multiple timecourses for each heading")
...
ggplot(data = avgtimecourse, aes(x = frame, y = meanSWA, colour = heading_factor)) + 
  geom_line()
