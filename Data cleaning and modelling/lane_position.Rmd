...
```{r computing lateral position per frame}


notproblem <- timecoursedata %>%
  dplyr::filter(WorldYaw < 315)

problem <- timecoursedata %>%
  dplyr::filter(WorldYaw >= 315) %>%
  mutate(WorldYaw = 360 - WorldYaw)

timecoursedata <- rbind(notproblem, problem)

timecoursedata <- timecoursedata %>%
  mutate(WorldYaw_rads = WorldYaw / 180 * pi)

# computes hypotenuse
timecoursedata <- timecoursedata %>%
  group_by(frame) %>%
  mutate(a = (World_x)^2 + (World_z)^2) %>%
  mutate(hypo = sqrt(a)) %>%
  mutate(steeringbias = sin(WorldYaw_rads) * hypo)
