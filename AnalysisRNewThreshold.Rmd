...
```{r averaging data and saving for magnitude matlab data}
library(dplyr)
library(tidyr)
...
avgtimecourse_collapsed <- avg_coll %>%
  group_by(heading_coll, frame) %>%
  summarise(meanYaw = mean(meanYaw))

  avgtimecourse <- spread(avgtimecourse, heading, meanYawRateChange) # spreads long to wide
# Write CSV in R
