...
```{r compute mean lateral error threshold for each heading according to threshold model}


ggplot(modellingdata %>%
         group_by(heading) %>%
         summarise(meanLateralPosition = mean(multilevel_threshold)), aes(x = heading, y = meanLateralPosition)) +
  ...
  ggtitle("Threshold model predictions of lateral position at first steering response trigger") 
...
# computes SWA across timecourse for each heading (or participant)
ggplot(timecoursedata, aes(x = timestamp_zero, y = SWA_mirrored, group = ppid_trialn, col = ppid_trialn)) +
  geom_line(alpha = 0.3) + 
  guides(colour = FALSE) +
  facet_wrap(~ pNum) +
  facet_wrap(~ abs_heading) +
  ggtitle("Steering wheel angle across multiple timecourses for each heading")

# computes average SWA across the time course
avgtimecourse <- timecoursedata %>%
  dplyr::filter(frame <= 270) %>%
  group_by(abs_heading, frame) %>%
  summarise(meanSWA = mean(SWA_mirrored), meanDiff_YR = mean(diff_YR_mirrored)) %>%
  mutate(heading_factor = as.factor(abs_heading)) %>%
  mutate(timestamp = frame / 60)

# computes SWA angle average over time for each heading
ggplot(data = avgtimecourse, aes(x = timestamp, y = meanSWA, colour = heading_factor)) + 
  geom_line() +
  theme_gray() +
  xlab("Time (s)") +
...
modellingdata %>%
  group_by(heading) %>%
  summarise(meanLateralPositionTHRESHOLD = mean(model_threshold))

# average threshold value for each error rate value according to data
dat_error <- dat %>%
  dplyr::filter(heading > 0) %>%
  group_by(heading) %>%
  summarise(meanLateralPositionDATA = mean(lane_position))
...
ggplot(dat %>%
  ...
  ylab("Lateral position from line (m)") +
  theme(axis.title.x = element_text(size = 15), axis.text.x = element_text(size = 15), axis.title.y = element_text(size = 15), axis.text.y = element_text(size = 15), title = element_text(size = 10)) +
  ggtitle("Lateral position from the road-line at first steering response trigger") 
