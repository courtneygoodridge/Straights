...
upperthreshold = 0.130 # 0.105 #  # upper threshold for consistent steering response
...
# spike trial remover 1

# b <- workingdatatimecourseUnsmooth %>% 
  # group_by(ppid_trialn) %>% # group by trials
  # filter(!(max(YawRateChange) > 0.8)) # removes group who max YawRateChange is greater than 1
       #! operator selects groups where the defined condition (max(YawRateChange) > 1) is not true
# View(b)

# spike trial remover 2

# threshold = 0.8
# grps_to_filt <- unique(workingdatatimecourseUnsmooth[workingdatatimecourseUnsmooth$YawRateChange > threshold, "ppid_trialn", drop = TRUE])
# c <- workingdatatimecourseUnsmooth[!workingdatatimecourseUnsmooth$ppid_trialn %in% grps_to_filt, , drop = FALSE]
# View(c)
```

```{r averaging data and saving for magnitude matlab data OLD}
library(dplyr)
library(tidyr)

avgtimecourse <- workingdatatimecourseUnsmooth %>%
  group_by(ppid_trialn) %>%
  filter(max(abs(YawRateChange)) > upperthreshold, min(abs(YawRateChange)) < lowerthreshold) %>%
  ungroup() %>%
  group_by(heading, frame) %>%
  summarise(meanYawRateChange = mean(YawRateChange))

negative_timecourse <- avgtimecourse %>%
  filter(heading < 0) %>%
  mutate(heading_coll = abs(heading)) %>%
  mutate(meanYaw = -(meanYawRateChange))

positive_timecourse <- avgtimecourse %>%
  filter(heading >= 0) %>%
  mutate(heading_coll = abs(heading)) %>%
  mutate(meanYaw = meanYawRateChange)

avgtimecourse_collapsed <- rbind(negative_timecourse, positive_timecourse)

avgtimecourse_collapsed <- avgtimecourse_collapsed %>%
  group_by(heading_coll, frame) %>%
  summarise(meanYaw = mean(meanYaw))

  avgtimecourse <- spread(avgtimecourse_collapsed, heading_coll, meanYaw) # spreads long to wide
# Write CSV in R
...
mag_response_start <- workingdatatimecourseUnsmooth %>% 
...
group_by(ppid_trialn) %>%
  ...
  slice(1:min(which(abs(YawRateChange) > upperthreshold, 1))) %>% # slice from the first yaw value, to the first yaw value above the upper thrshold
  slice(min(which(abs(YawRateChange) > lowerthreshold, 1))) %>% # slice the row at the minimum yaw rate above the lower threshold to get start of the response
  ungroup() %>%
  ...
  negative_magnitude <- mag_response_start %>%
  filter(heading < 0) %>%
  mutate(heading_coll = abs(heading)) %>%
  mutate(start_yaw = -(start_yaw))

  positive_magnitude <- mag_response_start %>%
  filter(heading >= 0) %>%
  mutate(heading_coll = abs(heading)) %>%
  mutate(start_yaw = (start_yaw))

  mag_response_start <- rbind(negative_magnitude, positive_magnitude)
...
```
```{r example code for plotting peak yaw rate on timecourse}
ggplot(data = filter(workingdatatimecourseUnsmooth, ppid_trialn == "1_2_1"), aes(x = anchored_timestamp, y = YawRateChange)) +
+ geom_smooth() +
+ geom_hline(data = filter(mag_response_peak, ppid_trialn == "1_2_1"), aes(yintercept = peak_yaw))
...
mag_response_peak <- workingdatatimecourseUnsmooth %>%
select(ppid_trialn, heading, frame, YawRateChange, anchored_timestamp) %>%
group_by(ppid_trialn) %>%
mutate(peakYaw = max(YawRateChange)) %>%
filter(max(abs(YawRateChange)) > upperthreshold, min(abs(YawRateChange)) < lowerthreshold) %>% 
slice(1:max(which(abs(YawRateChange) > upperthreshold, 1))) %>%
slice(max(which(abs(YawRateChange) == peakYaw))) %>%
ungroup() %>%
transmute(ppid_trialn, peak_frame = frame, peak_yaw = YawRateChange, peak_time = anchored_timestamp)

# inverts values
negative_magnitude <- mag_response_peak %>%
filter(heading < 0) %>%
mutate(heading_coll = abs(heading)) %>%
mutate(start_yaw = -(start_yaw))

positive_magnitude <- mag_response_peak %>%
filter(heading >= 0) %>%
mutate(heading_coll = abs(heading)) %>%
mutate(start_yaw = (start_yaw))

mag_response_peak <- rbind(negative_magnitude, positive_magnitude)
...
# collapsing conditions across headings

workingdatatimecourseUnsmooth <-  workingdatatimecourseUnsmooth %>%
  group_by(ppid_trialn) %>%
  mutate(heading_collapsed = abs(heading))
...
ggplot(data = filter(workingdatatimecourseUnsmooth, heading == -1.5), aes(x = anchored_timestamp, y = SWA, colour = ppid_trialn)) +
  geom_line(show.legend = FALSE) +
  geom_vline(data = filter(workingdatathresholdUnsmooth, heading == -1.5) %>%
               summarise(meanFirstSteeringTime = mean(FirstSteeringTime, na.rm = TRUE)), aes(xintercept = meanFirstSteeringTime)) +
  ggtitle("-1.5 heading - unsmoothed SWA")

######## -1 heading ##########

ggplot(data = filter(workingdatatimecourseUnsmooth, heading == -1), aes(x = anchored_timestamp, y = SWA, colour = ppid_trialn)) +
  geom_line(show.legend = FALSE) +
  geom_vline(data = filter(workingdatathresholdUnsmooth, heading == -1) %>%
               summarise(meanFirstSteeringTime = mean(FirstSteeringTime, na.rm = TRUE)), aes(xintercept = meanFirstSteeringTime)) +
  ggtitle("-1 heading - unsmoothed SWA")

######## -0.5 ###########

ggplot(data = filter(workingdatatimecourseUnsmooth, heading == -0.5), aes(x = anchored_timestamp, y = SWA, colour = ppid_trialn)) +
  geom_line(show.legend = FALSE) +
  geom_vline(data = filter(workingdatathresholdUnsmooth, heading == -0.5) %>%
               summarise(meanFirstSteeringTime = mean(FirstSteeringTime, na.rm = TRUE)), aes(xintercept = meanFirstSteeringTime)) +
  ggtitle("0.5 heading - unsmoothed SWA")
...
ggplot(data = filter(workingdatatimecourseUnsmooth, heading == -2), aes(x = anchored_timestamp, y = YawRateChange, colour = ppid_trialn)) +
  ...
  # scale_y_continuous(limits = c(-0.50, 0.75)) -> how to alter scale
...
```{r timecourse plotting of yaw rate change - SMOOTH}
library(ggplot2)
library(dplyr)
library(gganimate)
library(gifski)
...
workingdatathresholdSmooth <- workingdatathresholdSmooth %>%
  filter(Genuine_Response == TRUE & EarlyResponses == FALSE & Spike == FALSE)
...
```{r timecourse plotting for individual trials - SMOOTH, fig.height = 18, fig.width = 26}
library(ggplot2)
library(dplyr)
...
ggplot(data = filter(workingdatatimecourseSmooth, heading == 2), aes(x = anchored_timestamp, y = smoothedYawRateChange)) +
  geom_line() +
  geom_vline(data = filter(workingdatathresholdSmooth, heading == 2), aes(xintercept = FirstSteeringTime, color = EarlyResponses)) +
  facet_wrap( ~ ppid_trialn) +
  ggtitle("+2 heading - smoothed")
...
avgFirstSteeringTimes <- workingdatathresholdUnsmooth %>%
  filter(heading_collapsed > 0) %>%
  group_by(heading_collapsed) %>%
  summarise(humanRTdata = mean(FirstSteeringTime, na.rm = TRUE))

avgFirstSteeringTimes[1] <- NULL
