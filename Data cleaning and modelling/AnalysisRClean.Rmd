...
library(zoo) # package for rolling mean (rollmean function)
# library(skimr) # generates summary of data
library(car) # to perform levenes and anova
# library(apaTables) # to create apa table
# library(WRS2) # for non-parametric anova
# library(compute.es) # for ancova type 3 method
# library(multcomp) # post hoc for type 3 method ancova
# library(lmPerm) # non-parametric ancova
# library(e1071) # for calculating skewness
# library(ggpubr) # for interactions plots and boxplots
# library(effsize) # for calculating effect sizes
# library(gganimate) # animate plots
# library(gifski) # animate plots
# library(png)
# library(transformr)
library(signal) # package needed for sgolayfilt function for steering magnitude 
library(conflicted) # heloful package to show when packages have conflicted functions # conflict_prefer("filter", "dplyr") means filter function from dplyr is prioritised
...
female <- dplyr::filter(gender, gender == 1) 
male <- dplyr::filter(gender, gender == 2)
...
```{r  thresholds, removing trials with dropped frames, calculating yaw rate change, filtering yaw rate change}
library(dplyr)
library(tidyr)
...
workingdata <- workingdata %>%
  ...
  mutate(YawRateChange = sgolayfilt(YawRateChange, n = 11)) %>% # golay filter data
  ungroup()
...
dplyr::select(ppid_trialn, heading, frame, YawRateChange, anchored_timestamp, pNum, WorldYaw) %>% 
group_by(ppid_trialn) %>%
  ...
  ungroup() %>%
  transmute(ppid_trialn, heading, StartFrame = frame, StartYawRateChange = YawRateChange, FirstSteeringTime = anchored_timestamp, pNum = pNum, ResponseStartWorldYaw = WorldYaw) %>%
...
dplyr::select(ppid_trialn, heading, frame, YawRateChange, anchored_timestamp, pNum, WorldYaw, World_x, World_z) %>%
group_by(ppid_trialn) %>%
...
ungroup() %>%
transmute(ppid_trialn, heading, PeakFrame = frame, PeakYawRateChange = YawRateChange, PeakSteeringTime = anchored_timestamp, pNum = pNum, PeakWorldYaw = WorldYaw, PeakWorld_x = World_x, PeakWorld_z = World_z)
...
setwd("C:/Users/Courtney/Documents/PhD/Project/Experiment_code/Straights") # set directory for data saving
...
ggplot(data = trueworkingdata, aes(x = anchored_timestamp, y = YawRateChange, colour = ppid_trialn)) +
  ...
  facet_wrap( ~ heading) +
  scale_y_continuous(name="Yaw rate change", limits=c(-1, 1.5)) +
  scale_x_continuous(name="Timestamp (secs)", limits=c(0, 1.5)) +
  theme_gray()
...
# differentiated YawRateChange over time when line is visible
ggplot(trueworkingdata %>%
         dplyr::filter(frame <= 151) %>%
         group_by(heading, frame) %>%
         summarise(meanYawRateChange = mean(YawRateChange)) %>%
         mutate(heading_factor = as.factor(heading)) %>% 
         mutate(timestamp = frame / 60), aes(x = timestamp, y = meanYawRateChange, colour = heading_factor)) + 
  geom_line() +
  xlab("Time (s)") +
  ylab("Yaw Rate Change (degree/s/s)") +
  ...
  ggtitle("Average yaw rate change over time for each heading") +
  theme_gray()
...
ggplot(dplyr::filter(workingdatathresholdUnsmooth, heading > 0), aes(as.factor(heading), FirstSteeringTime)) +
  geom_boxplot() +
  stat_summary(fun.y = mean, aes(shape = "mean"), colour = "red", geom = "point") +
  scale_shape_manual("", values = c("mean" = "O")) +
  xlab("Heading (degrees)") +
  ylab("First steering reacton time (s)") +
  ggtitle("First steering response RTs across conditions") +
  theme(axis.title.x = element_text(size = 18), axis.text.x = element_text(size = 18), axis.title.y = element_text(size = 18), axis.text.y = element_text(size = 18), title = element_text(size = 18), legend.title = element_text(size = 18), legend.text = element_text(size = 18), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  ggsave('RTboxplot.png', plot = last_plot(), width = 24, height = 17, units = 'cm', dpi = 300, type = 'cairo')
...
```{r first steering time graphed example}
