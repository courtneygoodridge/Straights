...
temp = list.files(pattern = c("BenLui17_1", "*.csv")) # list all CSV files in the directory
...
workingdata <- workingdata %>%
  group_by(trialn) %>%
  mutate(gender = 2, age = 23, licenseTime = 73, contacts_glasses = FALSE) %>% # female = 1, male = 2; license time in months
  ungroup()
...
write.csv(workingdata, file = "BenLui17_1_full.csv")
