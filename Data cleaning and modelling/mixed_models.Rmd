...
ggplot(dplyr::filter(workingdata2, pNum == 1, heading == 2.0), aes(x = timestamp_zero, y = SWA_mirrored)) + #, group = ppid_trialn, col = ppid_trialn
  geom_line() + guides(colour = FALSE)

ggplot(dplyr::filter(workingdata2, ppid_trialn == "1_1_76"), aes(x = timestamp_zero, y =diff_YR)) +
         geom_line() + guides(colour = FALSE)

# plots x and z coordinates for all trials for a specific subject

ggplot(dplyr::filter(workingdata2, pNum == 11), aes(x = World_x, y = World_z)) + geom_point()
