...
modellingdata <- merge(modellingdata, trialstartcoords, by = "ppid_trialn")
...
trialendcoords <- workingdata %>%
  ...
  ungroup() %>%
  transmute(ppid_trialn, EndWorld_x = World_x, EndWorld_z = World_z, EndWorldYaw = WorldYaw) # save values
...
notproblem <- modellingdata %>%
  dplyr::filter(ComputedHeading < 350)

problem <- modellingdata %>%
  dplyr::filter(ComputedHeading >= 350) %>%
  mutate(ComputedHeading = 360 - ComputedHeading)

modellingdata <- rbind(notproblem, problem)
...
modellingdata <- modellingdata %>%
  mutate(data_lane_position = sin(ComputedHeading) *hypo)
...
modellingdata %>%
  group_by(heading) %>%
  summarise(meanLanePosition = mean(data_lane_position))
