...
temp = list.files(pattern = c("BenLui17_8", "*.csv")) # list all CSV files in the directory
...
workingdata <- workingdata %>%
  group_by(trialn) %>%
  mutate(gender = 1, age = 26, licenseTime = 109, contacts_glasses = TRUE) %>% # female = 1, male = 2; license time in months
  ungroup()
...
write.csv(workingdata, file = "BenLui17_8_full.csv")
