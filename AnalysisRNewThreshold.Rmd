...
initialSWA <- workingdatatimecourseUnsmooth %>%
  group_by(ppid_trialn) %>%
  summarise(initialSWA = head(SWA, n = 1))

workingdatathresholdUnsmooth <- merge(workingdatathresholdUnsmooth, initialSWA, by = "ppid_trialn")
...
# shapiro-wilk test for each heading - significant means not normally distrobuted
workingdatathresholdSmooth_heading2 <- filter(workingdatathresholdSmooth, heading == 2)
