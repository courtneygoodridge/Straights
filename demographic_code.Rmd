...
temp = list.files(pattern = c("BenLui17_11_", "*.csv")) # list all CSV files in the directory
...
workingdata <- workingdata %>%
  group_by(trialn) %>%
  mutate(gender = 1, age = 27, licenseTime = 108, contacts_glasses = TRUE, pNum = 11, handedness = 1) %>% # female = 1, male = 2; license time in months; right = 1, left = 2
  ungroup()
...
write.csv(workingdata, file = "BenLui17_11_full.csv")
