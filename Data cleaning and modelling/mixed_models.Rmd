...
modellingdata <- magnitudedata %>%
  ...
  mutate(error_rate = 1 / (sqrt(sin(heading_radians)) * 10)) %>%
...
for (i in c(1:19)){
  if (modellingdata$pNum == i){
    ...
    modellingdata$individual_slope <- heading_coefs[modellingdata$pNum]
    modellingdata$multilevel_slope <- heading_slope_m5[modellingdata$pNum]
  }
}
