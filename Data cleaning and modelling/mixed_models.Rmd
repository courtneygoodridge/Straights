...
modellingdata <- magnitudedata %>%
  ...
  mutate(FirstSteeringTime_log = log(FirstSteeringTime))
...
Taking the inverse of sin(heading) generates the metric error rate. Error rate refers to how much time it takes to travel a metre and thus, how slowly or quickly error develops. Conversely, ordinary sin(heading) generates error growth i.e. how much the error rate has developed. Plotting either of these metrics against the median of RT will show whether the relationship is linear or not.

Computing the inverse therefore makes the regression directtly interpretable - the intercept is the latency measured in seconds (i.e. perceptual processing time, motor and mechanical latency) whether the slope coefficient represents the threshold measured in metres. In this respect, is it the number of metres travelled away from the line before the threshold is surpassed as subjects react. 
...
ggplot(filter(workingdata2, pNum == 11, heading == 0), aes(x = timestamp_zero, y = SWA_mirrored, group = ppid_trialn, col = ppid_trialn)) +
         geom_line() + guides(colour = FALSE)

ggplot(filter(workingdata2, ppid_trialn == "9_2_69"), aes(x = timestamp_zero, y =diff_YR)) +
         geom_line() + guides(colour = FALSE)

# plots x and z coordinates for all trials for a specific subject

ggplot(filter(workingdata2, pNum == 11), aes(x = World_x, y = World_z)) + geom_point()
...
# model 4 - applying log normal distribution
m4 <- glmer(FirstSteeringTime ~ error_rate + (1 + error_rate | pNum), family = gaussian(link = log), data = modellingdata)
...
**m1** model formula specifies how FirstSteeringTime is explained by heading. The 1 specifies a varying intercept to vary by participant.

**m2** model forumula specifies how FirstSteeringTime is explained by heading, except (1 + heading | pNum) allows intercept and slope to vary between participants. Intercept and the slope of heading will vary by pNum. This accounts for subject variability in baseline fastest RTs (latency) and the subject variability in the sensitivity to heading conditions (threshold). 

**m3** using the generalised linear model which allows for other distributions to be used to model data. Data my suffer from heteroscadasity and thus cannot assume a normal distribution. **m3** therefore uses a gamma distribution to model data. 

**m4** using the generalise linear model which allows for other distributions to be applied to model data. Here I have attempted to apply a log-normal by applying the gaussian distribuiton  with the link function "log". The link function is applied to the data to make it log then the Gaussian is applied. Not much different from **m2** which is just a normal mixed effects model. 
...
predicted_values <- data.frame(fit = numeric(0), lwr = numeric(0), upr = numeric(0))

for (i in c(1:19)){
  # Create temporary data frame:
heading_tmp <- modellingdata[modellingdata$pNum == i,]
  # Perform regression:
reg_result <- lm(heading_tmp$FirstSteeringTime ~ heading_tmp$error_rate)
  # Get coefficient:
...
However, if each individual has a lot of data points per condition, we can be more certain of the individual coefficients. Thus we don't have to rely on the assumption of them being drawn from a normal distribution, and can extract individual regression lines rather than implement a mixed model.

**Difference between CIs and prediction intervals**

Confidence intervals tell you how well you have calculated the mean value. Imagine you sample from a Gaussian distribution a number of times and generate confidence intervals for each sample mean. You would expect 95% of those intervals to contain the population mean. Confidence intervals tell you about the likely location of the population parameter.

Prediction intervals tell you where you would expect to see the next data point that has been sampled for a new observation. Because they have to deal with a single point rather than the mean of values, predicition intervals are usually wider than confidence intervals.

One thing to note is that predicction intervals rely strongly on the assumption of constant variance. Hence re-computing these after the Box-Cox transformation might be a good idea.
